...
$$\text{gcd}(a, b) = \begin{cases}
   a &\text{, nếu } b = 0\\
   \text{gcd}(b, a \text{ mod } b) &\text{, trong trường hợp còn lại }
\end{cases}$$
...
Do vậy với ba số $a$, $b$, $a - b$, nếu một số $d$ bất kỳ là ước của một trong ba số trên thì sẽ là ước của hai số còn lại, tức là $\text{ƯC}(a, b) = \text{ƯC}(b, a - b)$. Điều này dẫn đến $\text{gcd}(a, b) = \text{gcd}(b, a - b)$.
...
- Để tính bội chung nhỏ nhất (BCNN) của hai số, ta dùng công thức: $\text{lcm}(a, b) = \frac{a \times b}{\text{gcd}(a, b)}$. Kể từ `C++17`, thư viện `numeric` cũng hỗ trợ cả hàm `	lcm(a, b)` cho phép tính BCNN của hai số.
